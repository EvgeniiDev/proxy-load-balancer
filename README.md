...
HTTP Proxy Load Balancer — это высокопроизводительный асинхронный инструмент для распределения HTTP/HTTPS трафика между несколькими HTTP прокси-серверами. Приложение автоматически отслеживает доступность прокси, перенаправляет запросы на работающие серверы и восстанавливает подключения к прокси после их восстановления.

## ✨ Функциональные возможности

### Основные функции:
- **Асинхронная архитектура**: Высокая производительность и масштабируемость
- **Протоколы**: Поддержка HTTP и HTTPS (через метод CONNECT)
...
- **Динамическая конфигурация**: Обновление настроек без перезапуска
...
async def main():
    ...
    await balancer.start()  # Запускает сервер и мониторинг
...
Массив HTTP прокси-серверов для балансировки:
...
### Производительность:
- **Высокая пропускная способность**: Обработка тысяч одновременных соединений
- **Низкая задержка**: Неблокирующие I/O операции
- **Эффективность ресурсов**: Минимальное потребление памяти и CPU

### Масштабируемость:
- **Concurrent connections**: Поддержка большого количества одновременных подключений
- **Event-driven architecture**: Эффективная обработка событий
- **Adaptive load handling**: Автоматическая адаптация к нагрузке
