...
Высокопроизводительный HTTP прокси-балансировщик для распределения нагрузки между SOCKS5 прокси-серверами с автоматическим мониторингом состояния и отказоустойчивостью.
...
### Основные функции:
- **Протоколы**: Полная поддержка HTTP и HTTPS (через метод CONNECT)
- **HTTP методы**: GET, POST, PUT, DELETE, PATCH, OPTIONS, HEAD и другие
...
- **Статистика**: Отслеживание успешных и неудачных запросов
- **Логирование**: Подробные логи работы с опциональным verbose-режимом
- **Конфигурируемые таймауты**: Настройка времени ожидания подключений
- **Управление сессиями**: Эффективное переиспользование соединений
...
- `PySocks==1.7.1` - для работы с SOCKS5 прокси
...
# Запуск с verbose и кастомным конфигом
python main.py -c custom_config.json -v
...
- **`load_balancing_algorithm`** - алгоритм балансировки нагрузки:
  ...
  - `"roundrobin"` или `"round_robin"` - циклический перебор прокси
...
### Основные команды:
```bash
# Запуск балансировщика с настройками по умолчанию
python main.py
...
#### Браузеры:
Настройте HTTP прокси в настройках сети вашего браузера:
- Прокси сервер: `localhost`
- Порт: `8080`
- Использовать для HTTP и HTTPS
...
Подробная информация об алгоритмах балансировки нагрузки и их настройке доступна в документе [LOAD_BALANCING.md](LOAD_BALANCING.md).

### Доступные алгоритмы:
- **Random** - случайный выбор прокси
- **Round Robin** - циклический перебор прокси
