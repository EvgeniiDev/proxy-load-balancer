...
HTTP/HTTPS Proxy Load Balancer — это усовершенствованный инструмент для распределения HTTP и HTTPS трафика между несколькими SOCKS5 прокси-серверами с возможностью полного анализа зашифрованного HTTPS трафика через SSL терминацию.

## ✨ Функциональные возможности

### Основные функции:
- **Протоколы**: Поддерживает HTTP и HTTPS (через метод CONNECT) прокси как входной протокол и SOCKS5 как выходной
- **HTTPS с SSL терминацией**: Новая возможность анализа HTTPS трафика с расшифровкой и статистикой HTTP статус кодов
...
### Новые возможности HTTPS:
- **SSL/TLS терминация**: Принимает зашифрованные HTTPS соединения от клиентов
- **Расшифровка трафика**: Получает расшифрованные HTTP данные для анализа
- **Анализ статус кодов**: Отслеживает HTTP статус коды ответов в HTTPS трафике
- **SNI анализ**: Автоматически определяет целевой хост из SNI в TLS handshake
- **Детальная статистика**: Ведет статистику HTTPS запросов, ответов и SSL handshakes
...
**Приложение запустит:**
- HTTP proxy на порту 8080 (по умолчанию)
- HTTPS proxy на порту 8443 (если включен)
- HTTPS Stats API на порту 8444 (если включен)
...
После запуска балансировщика настройте ваши приложения на использование прокси:

#### Традиционный HTTP/HTTPS Proxy:
...
#### HTTPS Proxy с SSL терминацией (новое):
- **HTTPS Proxy**: `https://localhost:8443` (если включен в конфигурации)
- **Статистика API**: `http://localhost:8444/stats`

### API эндпоинты для HTTPS статистики:

- `GET /stats` - Детальная статистика HTTPS трафика
- `GET /health` - Состояние HTTPS proxy
- `GET /reset` - Сброс статистики

```bash
# Получить статистику HTTPS трафика
curl http://localhost:8444/stats

# Проверить состояние HTTPS proxy
curl http://localhost:8444/health

# Сбросить статистику
curl http://localhost:8444/reset
```
